...
class DiscreteLog:
    def __init__(self, modulo, primroot, value):
        self.mod = modulo
        self.cyclic_group = GroupsTheory.Remainder_cyclic_group(self.mod)
        self.primitive_root = primroot
        self.value = value
    ...
    def hellman_pohlig_algorithm(self):
        ...
        # prime --> p
        # value --> a
        factorization = PrimalityTest.get_prime_factors(self.mod - 1, 2)
        for i in range(len(factorization)):
            ...
            while j <= order - 1:
                ...
                base_a = ExponentialTower.create_exp_tower(a_j, int((self.mod - 1) / factor ** (j + 1))).fast_exponentiation(self.mod)
                ...
                a_j = ModularCongruence.normalize(a_j * ExponentialTower.create_exp_tower(self.primitive_root, (-b_j * (factor ** j))).compute(self.mod), self.mod)
